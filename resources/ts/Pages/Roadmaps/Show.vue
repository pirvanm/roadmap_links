...
<template>
    ...
    <div class="relative min-h-screen bg-dots-darker bg-center bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white">
        ...
        <div v-show="showNodeInfo" class="absolute inset-0 min-h-screen z-[999999]">
            <div class="absolute inset-0 min-h-screen w-full bg-black opacity-20" @click="showNodeInfo = !showNodeInfo"></div>
                ...
                <div v-else>
                    ...
                    <div class="flex flex-col gap-1 text-sky-500">
                        <template v-if="nodeData?.links?.length">
                            <a :href="link.link" target="_blank" class="hover:underline" v-for="(link, linkIndex) in nodeData.links" :key="`link-${linkIndex}`">{{ link.title }}</a>
                        </template>
                        <template v-else>
